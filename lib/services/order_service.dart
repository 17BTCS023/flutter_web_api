...
class OrderService {
  static const API =
      "https://l5vqpgr2pd.execute-api.us-west-2.amazonaws.com/dev/api/v1/product/";
  ...
  Future<APIResponse<List<OrderForListing>>> getOrdersList() async {
    ...
    return http.get(API).then((data) {
      ...
      return APIResponse<List<OrderForListing>>(
          error: true, errorMessage: 'Some error occurred');
    }).catchError((_) => APIResponse<List<OrderForListing>>(
        error: true,
  ...
  Future<APIResponse<SingleOrder>> getOrder(int id) async {
    ...
    return http.get(API + id.toString()).then((data) {
      print(" ${data.statusCode}  ... *****************");
      ...
      return APIResponse<SingleOrder>(
          error: true, errorMessage: 'Some error occurred');
    }).catchError((_) => APIResponse<SingleOrder>(
        error: true,
        errorMessage: 'Some error occurred, caught in catch block'));
  }
  ...
  Future<APIResponse<bool>> createOrder(OrderManipulation item) async {
    ...
    return http
        .post(API, headers: {}, body: json.encode(item.toJson()))
        .then((data) {
  ...
  Future<APIResponse<bool>> deleteOrder(String id) async {
    ...
    return http.delete(API  + id + '/',
      ...
      print("STATUS CODE IS ${data.statusCode}  ... *****************");
      print("ERRORRR BODY : ${data.body}...........................");
      if (data.statusCode == 204) {
        // checking the status code
        print("${data.statusCode}........................");
        return APIResponse<bool>(data: true);
      ...
      return APIResponse<bool>(
        ...
        error: true, errorMessage: 'Some error occurred,couldnt delete '));
  }
}
